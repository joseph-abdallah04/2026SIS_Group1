  ...
  return (
    <div className="relative flex h-4.5 items-center gap-1.5 rounded-full border border-rt-secondary/20 bg-white px-2 shadow-sm">
      ...
      {/* Sized to the code rather than to the container: a `w-full` field in a
          flex row would stretch the header's whole right-hand group. */}
      <input
        ref={inputRef}
        readOnly
        value={code}
        ...
        className="w-[9ch] bg-transparent font-mono text-[11px] leading-none tracking-[0.06em] text-rt-ink outline-none"
      />
      <button
        type="button"
        onClick={() => void copy()}
