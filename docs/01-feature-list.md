...
| F10 | Late join while session in progress | Joiner receives current state snapshot and syncs. *Stretch — see S01* |
...
| F21 | Diagram tool: popup canvas editor with containers, elements, arrows, text → propose as diagram artifact | Simple node/edge model persisted as JSON; MVP = fixed element shapes |
