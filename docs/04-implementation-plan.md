...
- **Vertical slices over layers.** An engineer owns a module *end-to-end* (schema → API/socket events → UI), not "all the APIs" or "all the UI".
- **Walking skeleton first (Week 1).** Get an empty but *deployed* app — login screen → authenticated page → live socket echo — before features branch out. Integration risk dies early.
